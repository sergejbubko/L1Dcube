...
static const char index_html[] PROGMEM = "<html lang='en'>"
  ...
  "</head>"
  "<body>"
    "<header class='flex-column flex-md-row navbar' style='background-color: #333f50;color: #ffffff;'>"
      "<span style=\"font-family: 'Overpass', sans-serif;\">L1D cube Settings</span>"
    "</header>"
    "<div class='container'>"
      ...
      "<form action='/get' target='hidden-form' id='settingsForm'>"
        ...
        "<div class='mb-2' id='screenChangeDelay'>"
          ...
          "</div>"
        "</div>"
        "<div class='mb-2 form-check' id='autoUpdates'>"
          "<input type='checkbox' name='inputAutoUpdates' %INPUT_AUTO_UPDATES% class='form-check-input' value='on'>"
          "<label for='inputAutoUpdates' class='form-check-label'>Auto update at startup</label>"
        "</div>"
        "<div id='submitAlert' class='alert alert-success'>"
          "Settings saved. The cube will restart in few seconds."
        "</div>"
